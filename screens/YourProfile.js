...
export default function Profile() {
  ...
  const [chartMode, setChartMode] = useState("spending_history");
  ...
  const addNewCar = async (newId) => {
    console.log("Adding new car with id: ", newId);
    const stripedNewId = newId.replace(/\s/g, "");
    await fetch(`http://192.168.137.1:8000/api/carprofile/${stripedNewId}/`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((data) => {
        ...
        const allLocalCarData = localCarData;
        allLocalCarData.push(newCarData);
        CarStorage.writeItems(allLocalCarData);
        setLocalCarData(allLocalCarData);
        setModal(!isModalVisible);
      })
      .catch((error) => {
  ...
  const allChartMode = [
    { spending_history: "Spending History (THB)" }, // default
    { fuel_history: "Refuel Calendar" }, // count re fuel of each date
  ];

  const GenerateChart = () => {
    const GenerateSpedingHistory = () => {
      const groupedData = historyData.reduce((acc, item) => {
        acc[item.date] = (acc[item.date] || 0) + item.amount;
        return acc;
      }, {});
    ...
    if (chartMode === "spending_history") {
      return GenerateSpedingHistory();
    }
    if (chartMode === "fuel_history") {
      return GenerateFuelHistory();
    }
          ...
          >
              ...
              >
                ...
                <View style={{ flexDirection: "column" }}>
                  ...
                  <Picker
                    ...
                    onValueChange={(itemValue, itemIndex) =>
                      setChartMode(itemValue)
                    }
                  >
                    {allChartMode.map((item, index) => {
                      const key = Object.keys(item)[0];
                      const value = item[key];
                      return (
                        <Picker.Item label={value} value={key} key={index} />
                      );
                    })}
                  </Picker>
                  <GenerateChart />
            ...
            >
              ...
              >
                ...
                <Text style={{ color: "grey" }}> {carId}</Text>
              </View>
            </View>
          </LinearGradient>
        </View>
