...
export default function Profile() {
  ...
  const allChartMode = [
    ...
    { fuel_efficiency_history: "Liter / amount" }, // liter / amount
  ];
  ...
  const GenerateChart = () => {
    ...
    const GenerateFuelHistory = () => {
      const frequency = historyData.reduce((acc, item) => {
    ...
    const GenerateFuelEfficiencyHistory = () => {
      // bar chart liter / amount
      const data = {
        labels: historyData.map((item) => item.date),
        datasets: [
          {
            data: historyData.map((item) => item.amount / item.liter),
            color: (opacity = 1) => `rgba(134, 65, 244, ${opacity})`,
            strokeWidth: 2,
          },
        ],
    ...
    if (chartMode === "fuel_efficiency_history") {
      return GenerateFuelEfficiencyHistory();
    }
                      ...
                      <View style={{ flexDirection: "column", marginLeft: 20 }}>
                        <TouchableOpacity onPress={handleRemoveButton}>
                          ...
                          >
                            ...
                            <Text
                              style={{ color: "grey", marginHorizontal: 10 }}
                            >
                              Remove
                            </Text>
                          </View>
